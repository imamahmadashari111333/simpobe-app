    ...
    public function render()
{
    ...
    // 2. PERBAIKAN: Ambil semua CPMK milik prodi tanpa filter kode_cpl
    $list_cpmk = Cpmk::where('prodi', $this->prodi)
                ->orderBy('kode_cpmk')
                ->get();

    // 3. Filter Sub-CPMK berdasarkan pilihan CPMK (Agar tetap relevan)
    // Menggunakan where langsung ke kode_cpmk agar lebih akurat dibanding like regex
    $list_sub_cpmk = $this->kode_cpmk 
        ? SubCpmk::where('prodi', $this->prodi)
                 ->where('kode_sub_cpmk', 'like', '%' . preg_replace('/[^0-9]/', '', $this->kode_cpmk) . '%')
                 ->get() 
        : collect();

    // 4. Query Utama Relasi (Tetap)
    $this->relasis = RelasiCapaian::with([
            'mataKuliah', 'cpl', 'cpmk', 'subCpmk'
        ])
    ...
    return view('livewire.relasi-capaian-manager', [
        ...
        'list_cpmk' => $list_cpmk, // Mengirim data lengkap
        'list_sub_cpmk' => $list_sub_cpmk,
    ]);
}
